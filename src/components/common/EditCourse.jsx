...
const EditCourse = ({number, onClose}) => {
    ...
    useEffect(() => {
        const getCourse = async () => {
            const {response, err} = await courseApis.getCourse(number)
            if(response) {
                console.log(response)
                setCode(response.number)
                setName(response.name)
                setQuanlity(response.maxNumberOfStudents)
                setDepartment(response.department.id)
                setInstructor(response.instructorCourses.map((item) => ({
                    value: item.instructor.id,
                    label: `${item.instructor.fname} ${item.instructor.lname}`
                })))
            }
            if(err) {
                console.log(err)
    ...
    const saveClicked = async (event) => {
        ...
        if (!code || !name || quanlity === 0 || !department || !instructor) {
            ...
            console.log(data)
            const { response, err } = await courseApis.updateCourse(data)
            if (response) {
                console.log(response)
                toast.success("Cập nhật khoá học mới thành công !")
                onClose()
            }
            if (err) {
                console.log(err)
  ...
  return (
    <div className='main-edit-course'>
            <FontAwesomeIcon icon={faXmark} className="icon-editcourse" onClick={onClose} />
            <form className='form-edit-course' onSubmit={saveClicked}>
                ...
                <button className="save-edit-course" type="submit">
                    Save
                </button>
            </form>
        </div>
